...
function processVirabrequim($post, $ordem)
{
    ...
    $diametro_moente = sanitizeInput($post['diametro_moente'] ?? 0, 'float');
    $diametro_munhao = sanitizeInput($post['diametro_munhao'] ?? 0, 'float');

    $is_reference = 1;

    // Ajusta valores para o tipo bronzina
    if ($rolamento_type == 'bronzina') {
        // No caso de bronzina, alguns campos podem ser nulos se não forem preenchidos
    }

    // Verifica se existe um registro com a mesma ordem e is_reference = 1
    $recordId = checkRecordExists('virabrequim', 'ordem = ? AND is_reference = 1', [$ordem], 's');
    ...
    if ($recordId === null) {
        ...
        // Se for um update, usa a ordem e is_reference para a cláusula WHERE
        return processRecord('virabrequim', $fields, $values, $types, 'ordem = ? AND is_reference = 1', $ordem);
    }
}
